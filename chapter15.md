...
除了物理隔离，还需要从多个维度抑制串扰。光学串扰通过优化彩色滤光片（CF）和微透镜设计来减少，采用高消光比的CF材料，在非目标波长的透过率<0.1%。电学串扰通过优化掺杂分布和电场分布来控制，采用梯度掺杂将载流子引导到正确的像素。此外，还可以通过算法进行串扰补偿，基于预先标定的串扰矩阵，在ISP中进行反卷积处理。
